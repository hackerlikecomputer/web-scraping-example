...
def get_soup_with_retry(url):
    ...
    while retry_counter <= 10:
        ...
        msg = f'Request failed in line {getframeinfo(currentframe()).lineno} \
                after {retry_counter} attempts, waited a total of {total_wait_time/60} minutes'
        raise Timeout(msg)
...
for beat in tqdm(beats, position=0, desc='BEATS'):
    results = []
    url = f'http://publicsearch3.azurewebsites.net/\
            Arrests?Page=1&Beat={beat}&IsEmpty=True'
    ...
    if last_page_btn is not None:
        ...
        raise ScraperError(f'Scraper failed at line {getframeinfo(currentframe()).lineno}: \
                             Unable to assign max_page_no')
    for page in tqdm(range(1, max_page_no), position=1, desc='RESULTS PAGES'):
        page_url = f'http://publicsearch3.azurewebsites.net/\
                     Arrests?Page={page}&Beat={beat}&IsEmpty=True'
